...
// wms 2차
    ...
    results.forEach(item => {
        if (item.process === "COLLE") {
            if (item.priority === "1") {
                collec_info1.push(item);
            } else if (item.priority === "2") {
                collec_info2.push(item);
            }
        }
    });

    console.log('Priority 1:', collec_info1);
    console.log('Priority 2:', collec_info2);

//// collec ////
    const collecRow = table.insertRow();
    const collec = collecRow.insertCell();
    collec.textContent = "COLL";
    collec.rowSpan = 2;

    const collec_1 = collecRow.insertCell();
    collec_1.textContent = "1";

    const collec_1_N = collecRow.insertCell();
    collec_1_N.textContent = collec_info1.find(item => item.shift === "N")?.name || "";

    const collec_1_D = collecRow.insertCell();
    collec_1_D.textContent = collec_info1.find(item => item.shift === "D")?.name || "";

    const collec_1_E = collecRow.insertCell();
    collec_1_E.textContent = collec_info1.find(item => item.shift === "E")?.name || "";

// collec 2차
    const collecRow2 = table.insertRow();
    const collec_2 = collecRow2.insertCell();
    collec_2.textContent = "2";

    const collec_2_N = collecRow2.insertCell();
    collec_2_N.textContent = collec_info2.find(item => item.shift === "N")?.name || "";

    const collec_2_D = collecRow2.insertCell();
    collec_2_D.textContent = collec_info2.find(item => item.shift === "D")?.name || "";

    const collec_2_E = collecRow2.insertCell();
    collec_2_E.textContent = collec_info2.find(item => item.shift === "E")?.name || "";
